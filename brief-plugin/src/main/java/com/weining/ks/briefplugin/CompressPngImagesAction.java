...
//        progressDialog.show();
        filterAllPictures(mProject.getBaseDir());
    }

    private void filterAllPictures(VirtualFile rootFile) {
        ProgressManager.getInstance().run(new Task.Backgroundable(mProject, "图片压缩进度") {
            @Override
...
//                    if (selectedFile.isDirectory()) {
                        VfsUtilCore.visitChildrenRecursively(rootFile, new VirtualFileVisitor() {
                            ...
                            public boolean visitFile(@NotNull VirtualFile file) {
                                if (file.isDirectory() && file.getPath().contains("/build")) {
                                    return false;
                                }
                                if (!file.isDirectory()) {
                                    if (file.getPath().contains("src/main/res")) {
                                        if (file.getExtension() != null
                                            ...
                                            indicator.setText("采集图片 " + pictureFiles.size() + " " + file.getPath());
//                                            indicator.setText2(pictureFiles.size() + "");
                                        }
                                    }
                                }
                                return true;
...
//                            indicator.setText("采集图片 " + pictureFiles.size() + " " + selectedFile.getPath());
////                            indicator.setText2(pictureFiles.size() + "");
//                        }
//                    }
//                }
                ...
                for (int i = 0; i < pictureFiles.size(); i++) {
                    ...
                    try {
                        ...
                        indicator.setText("压缩图片 " + i + "/" + pictureFiles.size() + " " + virtualFile.getPath());
